...
def create_model_from_trial(trial: Trial, input_size: int, num_classes: int) -> CellTypeClassifier:
    ...
    for i in range(n_layers):
        max_size = 512 if i == 0 else hidden_sizes[i - 1]
...
def create_objective(X: np.ndarray, y: np.ndarray, num_classes: int,
                     n_folds: int = 3, num_epochs: int = 15, device: str = 'cpu'):
...
def tune_architecture(X: np.ndarray, y: np.ndarray, num_classes: int,
                      n_trials: int = 50, n_folds: int = 3, num_epochs: int = 15,
                      device: str = 'cpu', verbose: bool = True,
                      study_name: Optional[str] = None) -> Tuple[Dict[str, Any], optuna.Study]:
    """Run Optuna hyperparameter optimization to find the best architecture."""
    if verbose:
        print(f"\n{'='*60}")
        print("OPTUNA ARCHITECTURE TUNING")
        print(f"{'='*60}")
        print(f"  Trials: {n_trials}, CV Folds: {n_folds}, Max epochs: {num_epochs}")
        print(f"  Device: {device}, Input features: {X.shape[1]}, Classes: {num_classes}")
        print(f"{'='*60}\n")
    ...
    objective = create_objective(X, y, num_classes, n_folds=n_folds, num_epochs=num_epochs, device=device)
    ...
    if verbose:
        ...
        print(f"\n  Best Architecture: {hidden_sizes}")
        print(f"  Dropout: {best_params['dropout_rate']}, Attention: {best_params['use_attention']}")
        print(f"  LR: {best_params['learning_rate']:.6f}, WD: {best_params['weight_decay']:.6f}, Batch: {best_params['batch_size']}")
        print(f"{'='*60}\n")

    return best_params, study


def create_model_from_params(params: Dict[str, Any], input_size: int, num_classes: int) -> CellTypeClassifier:
    """Create a CellTypeClassifier from a params dictionary."""
